...
export default function InventoryManagementPage() {
  ...
  const { data: response, isLoading, refetch: refetchItems } = useGetInventoryItemsQuery({});
  const { data: usageResponse, isLoading: usageLoading, refetch: refetchUsage } = useGetInventoryUsageQuery({});
  const { data: summaryResponse, refetch: refetchSummary } = useGetInventorySummaryQuery({});
  const { data: suppliersResponse, refetch: refetchSuppliers } = useGetSuppliersQuery({});
  const { data: posResponse, refetch: refetchPOs } = useGetPurchaseOrdersQuery({});
      ...
      <div className="flex flex-1 flex-col lg:ml-[200px] ml-0 min-w-0">
        ...
        <main className="flex-1 flex flex-col pt-0 overflow-y-auto">
          <div className="w-full max-w-[1280px] px-8 py-8 mx-auto flex flex-col">
            ...
            <div className="flex items-center gap-1.5 text-[13px] text-muted-foreground font-semibold mb-4">
              <Link href="/admin/dashboard" className="hover:text-foreground transition-colors cursor-pointer text-primary">{mounted ? t('dashboard.title') : 'Dashboard'}</Link>
              <ChevronRight className="h-3.5 w-3.5 opacity-50" />
              <span className="text-foreground">{mounted ? t('inventoryPage.title') : 'Inventory Management'}</span>
            </div>
            ...
            <div className="flex flex-col gap-6 mb-8">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button
                  ...
                  title="Refresh Data"
                >
                  <RefreshCw className={`h-4 w-4 ${isLoading || usageLoading ? 'animate-spin' : ''}`} />
                </button>
                {user?.role !== 'TECHNICIAN' && (
                  <>
                    <button
                      onClick={() => setIsRequestStockOpen(true)}
                      className="flex items-center gap-2 h-10 px-4 rounded-lg border border-primary/50 bg-primary/10 text-sm font-black text-primary hover:bg-primary/20 transition-all shadow-sm focus:outline-none"
                    >
                ...
                <div className="relative">
                  ...
                  {isExportOpen && (
                    <div className="absolute top-12 left-0 w-44 bg-card rounded-xl shadow-xl border border-border py-1 z-[100] animate-in fade-in slide-in-from-top-2">
                      <button onClick={handleDownloadPDF} className="flex items-center gap-2 w-full px-4 py-2.5 text-left text-[13px] font-bold text-foreground hover:bg-muted transition-colors border-b border-border/50"><Download className="h-4 w-4 text-primary" /> {mounted ? t('reportsPage.downloadPdf') : 'Export as PDF'}</button>
                      <button onClick={handleExportCSV} className="flex items-center gap-2 w-full px-4 py-2.5 text-left text-[13px] font-bold text-foreground hover:bg-muted transition-colors"><Download className="h-4 w-4 text-emerald-500" /> {mounted ? t('reportsPage.downloadCsv') : 'Export as CSV'}</button>
                    </div>
                  )}
                </div>
                {user?.role !== 'TECHNICIAN' && (
            ...
            {/* Tab Switcher */}
            <div className="flex items-center gap-1 mb-8 bg-muted/50 p-1 rounded-2xl w-fit border border-border/40">
              <button
                onClick={() => setActiveTab("inventory")}
                className={`px-6 py-2.5 rounded-xl text-[13px] font-black transition-all ${activeTab === 'inventory' ? 'bg-card text-primary shadow-md' : 'text-muted-foreground hover:text-foreground'}`}
              >
                Stock Inventory
              </button>
              {user?.role !== 'TECHNICIAN' && (
                <>
                  <button
                    ...
                    className={`px-6 py-2.5 rounded-xl text-[13px] font-black transition-all ${activeTab === 'suppliers' ? 'bg-card text-primary shadow-md' : 'text-muted-foreground hover:text-foreground'}`}
                  >
                    Suppliers Hub
                  </button>
                  <button
                    onClick={() => setActiveTab("pos")}
                    className={`px-6 py-2.5 rounded-xl text-[13px] font-black transition-all ${activeTab === 'pos' ? 'bg-card text-primary shadow-md' : 'text-muted-foreground hover:text-foreground'}`}
                  >
                    Purchase Orders
                  </button>
                </>
            ...
            {activeTab === "inventory" && (
              <div className="w-full flex flex-col">
                <div className="mb-8">
                  <h2 className="text-lg font-bold text-foreground mb-4">{mounted ? t('inventoryPage.quickOverview') : 'Quick Overview'}</h2>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    {user?.role !== 'TECHNICIAN' && (
                      <div className="bg-card p-6 rounded-2xl border border-border/60 shadow-sm flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xl font-bold text-foreground">Rs. {(invSummary?.totalValue || 0).toLocaleString()}</span>
                        </div>
                      </div>
                    )}
                    <div className="bg-card p-6 rounded-2xl border border-border/60 shadow-sm flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xl font-bold text-foreground">{invSummary?.totalItems || 0} {mounted ? t('inventoryPage.skus') : 'SKUs'}</span>
                      </div>
                    </div>
                    <div className="bg-card p-6 rounded-2xl border border-border shadow-sm flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xl font-bold text-amber-500">{invSummary?.lowStockCount || 0} {mounted ? t('inventoryPage.items') : 'items'}</span>
                      </div>
                    </div>
                    <div className="bg-card p-6 rounded-2xl border border-border shadow-sm flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xl font-bold text-red-500">{invSummary?.outOfStockCount || 0} {mounted ? t('inventoryPage.items') : 'items'}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                  <div className="bg-card p-6 rounded-2xl border border-border shadow-sm flex flex-col">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="text-md font-bold text-foreground">{mounted ? t('inventoryPage.fastMovingItems') : 'Top 10 Fast-Moving Items'}</h3>
                      <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500 text-[10px] font-black uppercase tracking-widest">Live Ledger</span>
                    </div>
                    ...
                    <div className="mt-6 border-t border-border/50 pt-4">
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-[11px] font-black text-muted-foreground uppercase tracking-widest">Usage Ledger (Last 5)</span>
                      </div>
                      <div className="space-y-2">
                        {usageResponse?.usageReport?.slice(0, 5).map((r: any, idx: number) => (
                          <div key={idx} className="flex items-center justify-between p-2.5 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors border border-transparent hover:border-border">
                            <div className="flex items-center gap-3">
                              <div className="w-8 h-8 rounded-lg bg-card border border-border flex items-center justify-center text-[11px] font-black text-primary">
                                #{idx + 1}
                              </div>
                              <span className="text-[12px] font-bold text-foreground truncate max-w-[120px]">{r.partName}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-[12px] font-black text-primary">{r.totalQuantityUsed}</span>
                  ...
                  {user?.role !== 'TECHNICIAN' && (
                    <div className="bg-card p-6 rounded-2xl border border-border shadow-sm relative overflow-hidden group">
                      <div className="flex items-center justify-between mb-6">
                        <div>
                          <h3 className="text-md font-black text-foreground mb-1">{mounted ? t('inventoryPage.stockTrend') : 'Stock Value Trend'}</h3>
                          <p className="text-[11px] text-muted-foreground font-bold uppercase tracking-widest">{chartTimeRange}</p>
                        </div>
                        <select
                          value={chartTimeRange}
                          onChange={(e) => setChartTimeRange(e.target.value)}
                          className="h-8 rounded-lg border border-border bg-card px-2 text-[11px] font-black text-foreground focus:outline-none shadow-sm cursor-pointer"
                        >
                      ...
                      <div className="h-[280px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={stockTrendDataMap[chartTimeRange] || stockTrendDataMap["Last 6 months"]}>
                            ...
                            />
                            <Tooltip
                              contentStyle={{ borderRadius: '12px', border: '1px solid currentColor', backgroundColor: 'var(--card)', borderColor: 'var(--border)', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', padding: '12px' }}
                              itemStyle={{ fontSize: '12px', fontWeight: '800', color: 'var(--primary)' }}
                              labelStyle={{ fontSize: '10px', fontWeight: '700', color: 'var(--muted-foreground)', marginBottom: '4px', textTransform: 'uppercase' }}
                            />
                            <Area
                              type="monotone"
                              dataKey="value"
                ...
                <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden mb-8">
                  <div className="p-6">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
                      <div>
                        <h3 className="text-lg font-black text-foreground">{mounted ? t('inventoryPage.allInventoryItems') : 'All Inventory Items'}</h3>
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-3 mb-6">
                      <div className="relative flex-1 group">
                        <SearchIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground transition-colors group-focus-within:text-primary" />
                        <input
                          type="text"
                          placeholder="Search by name, SKU, brand..."
                          value={searchTerm}
                          onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1) }}
                          className="w-full h-11 pl-11 pr-4 rounded-xl border border-border bg-muted/30 text-[13px] font-bold focus:outline-none focus:ring-2 focus:ring-primary/10 focus:bg-card transition-all shadow-sm placeholder:text-muted-foreground"
                        />
                      </div>
                      <div className="flex items-center gap-2 w-full md:w-auto">
                        <select
                          className="h-11 px-4 rounded-xl border border-border bg-card text-[13px] font-bold text-foreground focus:outline-none focus:ring-2 focus:ring-primary/10 min-w-[150px] cursor-pointer shadow-sm hover:bg-muted transition-colors"
                        ...
                        <select
                          className="h-11 px-4 rounded-xl border border-border bg-card text-[13px] font-bold text-foreground focus:outline-none focus:ring-2 focus:ring-primary/10 min-w-[130px] cursor-pointer shadow-sm hover:bg-muted transition-colors"
                          value={filterStatus}
                      ...
                      <table className="w-full text-left border-collapse min-w-[1100px]">
                        <thead>
                          <tr className="bg-muted/30 border-b border-border text-[11px] font-black text-muted-foreground uppercase tracking-widest">
                            ...
                            {user?.role !== 'TECHNICIAN' && <th className="px-5 py-4">{mounted ? t('inventoryPage.table.unitPrice') : 'Unit Price'}</th>}
                        ...
                        <tbody className="divide-y divide-border/50">
                          {paginatedData.map((item) => (
                            <tr key={item.code} className={`group hover:bg-muted/30 transition-colors ${selectedItems.includes(item.code) ? 'bg-primary/5' : ''}`}>
                              ...
                              <td className="px-5 py-5">
                                <div className="flex flex-col gap-0.5">
                                  <span className="text-[13px] font-black text-foreground leading-tight">{item.name}</span>
                                  <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-black text-muted-foreground uppercase tracking-tighter">{item.code}</span>
                                    <span className="h-1 w-1 rounded-full bg-border" />
                                    <span className="text-[10px] font-bold text-primary uppercase">{item.brand}</span>
                                    <span className="h-1 w-1 rounded-full bg-border" />
                                    <span className="text-[10px] font-bold text-muted-foreground">{item.category}</span>
                                  </div>
                                </div>
                              </td>
                              <td className="px-5 py-5">
                                <div className="flex flex-col gap-1.5 min-w-[140px]">
                                  <div className="flex items-center justify-between">
                                    <span className="text-[12px] font-black text-foreground tracking-tight">{item.stock} units <span className="text-[10px] text-muted-foreground font-bold uppercase ml-1">Current</span></span>
                                  </div>
                                  <div className="w-full h-1.5 bg-primary/10 rounded-full overflow-hidden shadow-inner">
                                    <div
                                      className={`h-full rounded-full transition-all duration-700 shadow-sm ${item.status === 'In Stock' ? 'bg-emerald-500' :
                              ...
                              {user?.role !== 'TECHNICIAN' && (
                                <td className="px-5 py-5">
                                  <div className="flex flex-col">
                                    <span className="text-[13px] font-black text-foreground">Rs. {(item.price || 0).toLocaleString()}</span>
                                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">MSRP Value</span>
                                  </div>
                                </td>
                              )}
                              <td className="px-5 py-5">
                                <div className="flex flex-col gap-1">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[12px] font-bold text-foreground leading-none">{item.supplier}</span>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <MapPin className="h-3 w-3 text-primary" />
                                    <span className="text-[11px] font-black text-muted-foreground leading-none">Bay {item.location}</span>
                                  </div>
                                </div>
                              </td>
                              <td className="px-5 py-5 text-right">
                                <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border shadow-sm ${item.status === 'In Stock' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' :
                                ...
                                {activeMenuId === item.code && (
                                  <div className="absolute right-0 top-full mt-2 w-48 bg-card rounded-2xl shadow-2xl border border-border p-1.5 z-[100] animate-in fade-in zoom-in-95 origin-top-right">
                                    <button
                                      ...
                                      className="flex items-center gap-2.5 w-full px-4 py-2.5 text-[12px] font-bold text-foreground hover:bg-muted rounded-xl transition-colors"
                                    >
                                    ...
                                    <button
                                      ...
                                      className="flex items-center gap-2.5 w-full px-4 py-2.5 text-[12px] font-bold text-primary hover:bg-primary/5 rounded-xl transition-colors"
                                    >
                                    ...
                                    {user?.role !== 'TECHNICIAN' && (
                                      <>
                                        ...
                                        <button
                                          ...
                                          className="flex items-center gap-2.5 w-full px-4 py-2.5 text-[12px] font-bold text-foreground hover:bg-muted rounded-xl transition-colors"
                                        >
                                          <Settings className="h-4 w-4 text-muted-foreground" /> Edit Item
                                        </button>
                                        <button
                                          onClick={() => { setDeleteFormTarget(item); setActiveMenuId(null); }}
                                          className="flex items-center gap-2.5 w-full px-4 py-2.5 text-[12px] font-bold text-red-500 hover:bg-red-500/10 rounded-xl transition-colors"
                                        >
                    ...
                    {/* Pagination */}
                    <div className="flex items-center justify-between py-6 mt-4 border-t border-border">
                      <div className="flex items-center gap-2">
                        <span className="text-[13px] text-muted-foreground">{mounted ? t('inventoryPage.pagination.show') : 'Show'}</span>
                        <select className="h-8 px-2 rounded border border-border bg-card text-foreground text-[13px] font-bold focus:outline-none">
                          <option>10</option>
                          <option>25</option>
                          <option>50</option>
                        </select>
            ...
            {activeTab === "suppliers" && (
              <div className="w-full flex flex-col">
                <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden mb-8">
                  <div className="p-6">
                    <h3 className="text-lg font-black text-foreground mb-6">Suppliers Directory</h3>
                    <div className="w-full overflow-x-auto rounded-xl border border-border/50">
                        ...
                        <tbody className="divide-y divide-border/50">
                          {suppliers.map((s: any) => (
                            <tr key={s.id} className="hover:bg-muted/30 transition-colors">
                              <td className="px-5 py-4 text-[13px] font-black text-foreground">{s.name}</td>
                              <td className="px-5 py-4 text-[13px] font-bold text-muted-foreground">{s.contactName || '-'}</td>
                              <td className="px-5 py-4 text-[13px] text-muted-foreground">{s.email || '-'}</td>
                              <td className="px-5 py-4 text-[13px] text-muted-foreground">{s.phone || '-'}</td>
                              <td className="px-5 py-4"><span className="px-2 py-1 rounded-lg bg-muted text-[10px] font-black uppercase">{s.category}</span></td>
                              <td className="px-5 py-4 text-center relative">
                                <div className="relative inline-block">
                                  ...
                                  {activeSupplierMenuId === s.id && (
                                    <div className="absolute right-0 top-full mt-2 w-44 bg-card rounded-2xl shadow-2xl border border-border p-1.5 z-[100] animate-in fade-in zoom-in-95 origin-top-right">
                                      <button
                                        onClick={() => { setEditSupplierTarget(s); setActiveSupplierMenuId(null); }}
                                        className="flex items-center gap-2.5 w-full px-4 py-2.5 text-[12px] font-bold text-foreground hover:bg-muted rounded-xl transition-colors"
                                      >
                                        <RefreshCw className="h-4 w-4 text-primary" /> Edit Supplier
                                      </button>
                                      <div className="h-px bg-border my-1 mx-2" />
                                      <button
                          ...
                          {suppliers.length === 0 && (
                            <tr><td colSpan={6} className="px-5 py-20 text-center text-muted-foreground font-bold">No suppliers registered yet.</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
            ...
            {activeTab === "pos" && (
              <div className="w-full flex flex-col">
                <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden mb-8">
                  <div className="p-6">
                    <h3 className="text-lg font-black text-foreground mb-6">Purchase Orders History</h3>
                    <div className="w-full overflow-x-auto rounded-xl border border-border/50">
                        ...
                        <tbody className="divide-y divide-border/50">
                          {purchaseOrders.map((po: any) => (
                            <tr key={po.id} className="hover:bg-muted/30 transition-colors">
                              <td className="px-5 py-4 text-[13px] font-black text-foreground">{po.orderNumber}</td>
                              <td className="px-5 py-4 text-[13px] font-bold text-muted-foreground">{po.supplier?.name || '-'}</td>
                              <td className="px-5 py-4 text-[13px] font-black text-foreground">Rs. {Number(po.totalAmount).toLocaleString()}</td>
                              <td className="px-5 py-4">
                                <select
                                  ...
                                  className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase border-0 outline-none cursor-pointer ${po.status === 'RECEIVED' ? 'bg-emerald-500/10 text-emerald-500' :
                                      po.status === 'SENT' ? 'bg-blue-500/10 text-blue-500' :
                                        po.status === 'CANCELLED' ? 'bg-red-500/10 text-red-500' : 'bg-muted text-muted-foreground'
                                    }`}
                                >
                              ...
                              <td className="px-5 py-4 text-[13px] text-muted-foreground">{new Date(po.createdAt).toLocaleDateString()}</td>
                              <td className="px-5 py-4 text-center relative">
                                <div className="flex items-center justify-center gap-2">
                                  <button
                                    onClick={() => setViewInvoiceTarget(po)}
                                    className="h-8 w-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all shadow-sm"
                                    title="View Invoice"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="px-4 py-1.5 rounded-lg bg-primary text-white text-[11px] font-black uppercase hover:bg-primary/90 transition-colors"
                                  >
                                    View
                                  </button>
                                  <div className="relative">
                                    <button
                                      ...
                                      className="h-8 w-8 rounded-lg bg-muted flex items-center justify-center text-muted-foreground hover:bg-primary/10 hover:text-primary transition-all focus:outline-none"
                                    >
                                      <MoreVertical className="h-4 w-4" />
                                    </button>
                                    ...
                                    {activePOMenuId === po.id && (
                                      <div className="absolute right-0 top-full mt-2 w-48 bg-card rounded-2xl shadow-2xl border border-border p-1.5 z-[100] animate-in fade-in zoom-in-95 origin-top-right">
                                        <button
                                          ...
                                          className="flex items-center gap-2.5 w-full px-4 py-2.5 text-[12px] font-bold text-foreground hover:bg-muted rounded-xl transition-colors disabled:opacity-50"
                                        >
                                        ...
                                        <button
                                          ...
                                          className="flex items-center gap-2.5 w-full px-4 py-2.5 text-[12px] font-bold text-foreground hover:bg-muted rounded-xl transition-colors"
                                        >
                                          <RefreshCw className="h-4 w-4 text-muted-foreground" /> Edit Order
                                        </button>
                                        <button
                          ...
                          {purchaseOrders.length === 0 && (
                            <tr><td colSpan={6} className="px-5 py-20 text-center text-muted-foreground font-bold">No purchase orders found.</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
                ...
                <div className="w-full max-w-[800px] bg-white shadow-sm border border-slate-200 p-16 flex flex-col min-h-[1050px] relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-20">
                    ...
                    <div className="text-right text-[11px] text-slate-400 font-black uppercase tracking-widest leading-relaxed">
                      <p className="Premium Service Center"></p>
        ...
        <div className="fixed -left-[9999px] top-0 pointer-events-none opacity-0 select-none z-[-1]">
          {viewInvoiceTarget && (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="space-y-8">
                  {viewInvoiceTarget.items?.map((item: any, idx: number) => (
                    <div key={idx} className="grid grid-cols-12 items-center">
                      <div className="col-span-6">
                        <p className="text-[14px] font-black text-foreground mb-1">{item.partName}</p>
                        <p className="text-[11px] text-muted-foreground font-bold uppercase tracking-wider">Primary Technical Asset Profile</p>
                      </div>
                      <div className="col-span-2 text-[13px] font-black text-foreground text-center uppercase">{viewInvoiceTarget.supplier?.name?.split(' ')[0] || "Apple"}</div>
                      <div className="col-span-2 text-[13px] font-black text-foreground text-center font-mono">{item.sku || "qqqqqqqqqqqqqq"}</div>
                      <div className="col-span-2 text-right text-[13px] font-black text-foreground">Rs. {(item.unitCost * item.quantity).toLocaleString()}</div>
                    </div>
                  ))}
                </div>

                {/* FINANCIAL TOTALS */}
                <div className="flex justify-end pt-12 mt-12 border-t-4 border-muted">
                  <div className="w-[340px] space-y-4">
                    <div className="flex justify-between items-center text-[13px] font-bold text-muted-foreground">
                      <span>Market Baseline Net</span>
                      <span className="text-foreground">Rs. {(Number(viewInvoiceTarget.totalAmount) * 0.9).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center text-[13px] font-bold text-muted-foreground pb-5 border-b border-border">
                      <span>Valuation Fee (10.0%)</span>
                      <span className="text-foreground">Rs. {(Number(viewInvoiceTarget.totalAmount) * 0.1).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center pt-2">
                      <span className="text-[16px] font-black text-foreground uppercase tracking-tighter">Total Asset Valuation</span>
                      <div className="text-right">
                        <p className="text-[24px] font-black text-primary tracking-tighter leading-none">Rs. {(Number(viewInvoiceTarget.totalAmount)).toLocaleString()}</p>
                        <p className="text-[9px] text-muted-foreground font-black uppercase mt-1">Authorized for Inventory</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* PROFESSIONAL FOOTER */}
              <div className="mt-20 pt-16 border-t border-border border-dashed">
                <p className="text-[12px] font-black text-foreground mb-8 flex items-center gap-2">
        ...
        {isAddSupplierOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-xl rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-emerald-500 max-h-[90vh] flex flex-col">
              <div className="bg-muted/30 p-6 border-b border-border flex justify-between items-center">
                <div>
                  <h2 className="text-[22px] font-black text-foreground mb-1 leading-none tracking-tight">Add New Supplier</h2>
                  <p className="text-[11px] text-emerald-500 font-black uppercase tracking-widest">Register a new vendor to the system</p>
                </div>
                <button onClick={() => setIsAddSupplierOpen(false)} className="h-9 w-9 rounded-full bg-card border border-border flex items-center justify-center transition-all focus:outline-none"><X className="h-4 w-4" /></button>
              </div>
              <div className="p-6 space-y-3 overflow-y-auto flex-1">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="col-span-2">
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Company Name</label>
                    <input value={supplierForm.name} onChange={e => setSupplierForm(p => ({ ...p, name: e.target.value }))} type="text" placeholder="e.g. Tech Supplies Inc" className="w-full h-10 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 outline-none transition-all" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Contact Person</label>
                    <input value={supplierForm.contactName} onChange={e => setSupplierForm(p => ({ ...p, contactName: e.target.value }))} type="text" placeholder="e.g. John Doe" className="w-full h-10 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 outline-none transition-all" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Phone Number</label>
                    <input value={supplierForm.phone} onChange={e => setSupplierForm(p => ({ ...p, phone: e.target.value }))} type="text" placeholder="+94 77 123 4567" className="w-full h-10 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 outline-none transition-all" />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Email Address</label>
                    <input value={supplierForm.email} onChange={e => setSupplierForm(p => ({ ...p, email: e.target.value }))} type="email" placeholder="contact@supplier.com" className="w-full h-10 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 outline-none transition-all" />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Category</label>
                    <select
                      ...
                      className="w-full h-10 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 outline-none transition-all bg-muted/30"
                    >
                  ...
                  {!["Parts", "Tools", "Electronics", "Accessories"].includes(supplierForm.category) && (
                    <div className="col-span-2 animate-in slide-in-from-top-2 duration-200">
                      <label className="block text-[11px] font-black text-emerald-500 uppercase tracking-widest mb-2">Specify Other Category</label>
                      <input
                        value={supplierForm.category}
                        onChange={e => setSupplierForm(p => ({ ...p, category: e.target.value }))}
                        type="text"
                        placeholder="e.g. Batteries, Packing Materials..."
                        className="w-full h-10 rounded-xl border-2 border-emerald-500/20 px-4 text-[13px] font-bold focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 outline-none transition-all bg-emerald-500/5"
                      />
                    </div>
                  )}
                  <div className="col-span-2">
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Business Address</label>
                    <textarea value={supplierForm.address} onChange={e => setSupplierForm(p => ({ ...p, address: e.target.value }))} placeholder="Company physical address..." className="w-full h-16 rounded-xl border border-border p-3 text-[13px] font-bold focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 outline-none transition-all resize-none" />
                  </div>
                </div>
                <div className="pt-6">
                  <button onClick={handleAddSupplier} className="w-full h-10 rounded-2xl bg-emerald-500 text-white text-[15px] font-black shadow-xl shadow-emerald-500/20 hover:bg-emerald-600 transition-all uppercase tracking-tight">Register Supplier</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {editSupplierTarget && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-xl rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-primary max-h-[90vh] flex flex-col">
              <div className="bg-muted/30 p-8 border-b border-border flex justify-between items-center">
                <div>
                  <h2 className="text-[22px] font-black text-foreground mb-1 leading-none tracking-tight">Edit Supplier</h2>
                  <p className="text-[11px] text-primary font-black uppercase tracking-widest">Update vendor contact and logistics data</p>
                </div>
                <button onClick={() => setEditSupplierTarget(null)} className="h-9 w-9 rounded-full bg-card border border-border flex items-center justify-center transition-all focus:outline-none"><X className="h-4 w-4" /></button>
              </div>
              <div className="p-6 space-y-3 overflow-y-auto flex-1">
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Company Name</label>
                    <input value={editSupplierTarget.name} onChange={e => setEditSupplierTarget(p => ({ ...p, name: e.target.value }))} type="text" className="w-full h-10 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none transition-all" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Contact Person</label>
                    <input value={editSupplierTarget.contactName} onChange={e => setEditSupplierTarget(p => ({ ...p, contactName: e.target.value }))} type="text" className="w-full h-10 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none transition-all" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Phone Number</label>
                    <input value={editSupplierTarget.phone} onChange={e => setEditSupplierTarget(p => ({ ...p, phone: e.target.value }))} type="text" className="w-full h-10 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none transition-all" />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Email Address</label>
                    <input value={editSupplierTarget.email} onChange={e => setEditSupplierTarget(p => ({ ...p, email: e.target.value }))} type="email" className="w-full h-10 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none transition-all" />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Category</label>
                    <select
                      ...
                      className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none transition-all bg-muted/30"
                    >
                      <option value="Parts">Parts</option>
                      <option value="Tools">Tools</option>
                      <option value="Electronics">Electronics</option>
        ...
        {deleteSupplierTarget && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-sm rounded-[32px] shadow-2xl overflow-hidden p-8 text-center animate-in zoom-in-95 duration-200 border border-border">
              <div className="h-16 w-16 bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-6">
                <AlertTriangle className="h-8 w-8 text-red-500" />
              </div>
              <h2 className="text-[20px] font-black text-foreground mb-2 leading-none tracking-tight">Remove Supplier?</h2>
              <p className="text-[13px] text-muted-foreground font-bold mb-8">Are you sure you want to delete <span className="text-foreground font-black">{deleteSupplierTarget.name}</span>? This action is irreversible.</p>
              <div className="flex flex-col gap-3">
                <button onClick={handleDeleteSupplier} className="h-12 rounded-xl bg-red-500 text-white text-[14px] font-black shadow-xl shadow-red-500/20 hover:bg-red-600 transition-all uppercase">Confirm Deletion</button>
                <button onClick={() => setDeleteSupplierTarget(null)} className="h-12 rounded-xl border border-border text-[14px] font-black text-muted-foreground hover:bg-muted transition-all uppercase">Cancel</button>
              </div>
            </div>
          </div>
        )}
        ...
        {deletePOTarget && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-sm rounded-[32px] shadow-2xl overflow-hidden p-8 text-center animate-in zoom-in-95 duration-200 border border-border">
              ...
              <h2 className="text-[20px] font-black text-foreground mb-2 leading-none tracking-tight">Delete Order #{deletePOTarget.orderNumber}?</h2>
              <p className="text-[13px] text-muted-foreground font-bold mb-8">This will permanently remove this purchase record. Ensure this action is authorized.</p>
              <div className="flex flex-col gap-3">
                <button onClick={handleDeletePO} className="h-12 rounded-xl bg-red-500 text-white text-[14px] font-black shadow-xl shadow-red-500/20 hover:bg-red-600 transition-all uppercase">Confirm Delete</button>
                <button onClick={() => setDeletePOTarget(null)} className="h-12 rounded-xl border border-border text-[14px] font-black text-muted-foreground hover:bg-muted transition-all uppercase">Cancel</button>
              </div>
            </div>
          </div>
        )}
        ...
        {editPOTarget && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-primary">
              <div className="bg-muted/30 p-8 border-b border-border flex justify-between items-center">
                <div>
                  <h2 className="text-[24px] font-black text-foreground mb-1 leading-none tracking-tight">Edit Order #{editPOTarget.orderNumber}</h2>
                  <p className="text-[12px] text-primary font-black uppercase tracking-widest">Update order status and administrative notes</p>
                </div>
                <button onClick={() => setEditPOTarget(null)} className="h-9 w-9 rounded-full bg-card border border-border flex items-center justify-center transition-all focus:outline-none"><X className="h-4 w-4" /></button>
              </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full h-12 rounded-xl border border-border px-4 text-[13px] font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none transition-all bg-muted/30"
                    >
                      <option value="DRAFT">DRAFT</option>
                      <option value="SENT">SENT</option>
                      <option value="RECEIVED">RECEIVED</option>
                  ...
                  <div>
                    <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-2">Supplier (Read-Only)</label>
                    <input value={editPOTarget.supplier?.name} disabled className="w-full h-12 rounded-xl border border-border px-4 text-[13px] font-bold bg-muted text-muted-foreground" />
                  </div>
                  <div className="col-span-2">
        ...
        {isRequestStockOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-primary">
              <div className="bg-muted/30 p-8 border-b border-border flex justify-between items-center">
                <div>
                  <h2 className="text-[24px] font-black text-foreground mb-1 leading-none tracking-tight">Request Stock (Purchase Order)</h2>
                  <p className="text-[12px] text-primary font-black uppercase tracking-widest">Draft a new order for inventory replenishment</p>
                </div>
                <button onClick={() => setIsRequestStockOpen(false)} className="h-9 w-9 rounded-full bg-card border border-border flex items-center justify-center transition-all focus:outline-none"><X className="h-4 w-4" /></button>
              </div>
              <div className="p-10 space-y-6 max-h-[70vh] overflow-y-auto">
                ...
                <div className="space-y-4">
                  ...
                  {poDraftForm.items.map((item, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-3 p-4 bg-muted/30 rounded-2xl border border-border/50">
                      <div className="col-span-6">
        ...
        {isAddItemOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-primary max-h-[90vh] flex flex-col">
              <div className="bg-muted/30 p-6 border-b border-border flex justify-between items-center">
                <div>
                  <h2 className="text-[22px] font-black text-foreground mb-1 leading-none tracking-tight">{mounted ? t('inventoryPage.registerItem') : 'Register Inventory Item'}</h2>
                  <p className="text-[11px] text-primary font-black uppercase tracking-widest">{mounted ? t('inventoryPage.createSkuRecord') : 'Create a new SKU record in the system'}</p>
                </div>
                <button onClick={() => setIsAddItemOpen(false)} className="h-9 w-9 rounded-full bg-card border border-border flex items-center justify-center hover:bg-muted transition-all focus:outline-none"><X className="h-4 w-4" /></button>
              </div>
        ...
        {editItemTarget && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-primary max-h-[90vh] flex flex-col">
              <div className="bg-muted/30 p-6 border-b border-border flex justify-between items-center">
                <div>
                  <h2 className="text-[22px] font-black text-foreground mb-1 leading-none tracking-tight">{mounted ? t('inventoryPage.editSkuRecord') : 'Edit SKU Record'}</h2>
                  <p className="text-[11px] text-primary font-black uppercase tracking-widest">Editing: {editItemTarget.code}</p>
                </div>
                <button onClick={() => setEditItemTarget(null)} className="h-9 w-9 rounded-full bg-card border border-border flex items-center justify-center transition-all focus:outline-none"><X className="h-4 w-4" /></button>
              </div>
              <div className="p-8 grid grid-cols-2 gap-x-8 gap-y-4 overflow-y-auto flex-1">
                <div className="col-span-2">
        ...
        {viewDetailsTarget && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-lg rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-border">
              <div className="bg-muted/30 p-8 border-b border-border flex justify-between items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-[20px] font-black text-foreground leading-tight">{viewDetailsTarget.name}</h2>
                    <p className="text-[11px] text-primary font-black uppercase tracking-widest">{viewDetailsTarget.code}</p>
                  </div>
                </div>
                <button onClick={() => setViewDetailsTarget(null)} className="h-10 w-10 rounded-full bg-card border border-border flex items-center justify-center hover:bg-muted transition-all focus:outline-none"><X className="h-5 w-5" /></button>
              </div>
              <div className="p-8">
                <div className="grid grid-cols-2 gap-6 mb-8">
                  <div className="bg-muted/30 p-4 rounded-2xl border border-border/50">
                    <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mb-1">Current Stock</p>
                    <p className="text-[20px] font-black text-foreground">{viewDetailsTarget.stock} Units</p>
                  </div>
                  <div className="bg-muted/30 p-4 rounded-2xl border border-border/50">
                    <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mb-1">Selling Price</p>
                    <p className="text-[20px] font-black text-emerald-500">Rs. {viewDetailsTarget.price.toLocaleString()}</p>
                  </div>
                </div>
                <div className="space-y-4">
        ...
        {isAdjustStockOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-primary">
              <div className="bg-muted/30 p-8 border-b border-border">
                <h2 className="text-[24px] font-black text-foreground mb-1 leading-none tracking-tight">Quick Stock Adjust</h2>
                <p className="text-[12px] text-primary font-black uppercase tracking-widest">Update inventory ledger without full edit</p>
              </div>
              <div className="p-10 space-y-6">
                <div>
                ...
                <div className="pt-4 flex gap-4">
                  <button onClick={() => setIsAdjustStockOpen(false)} className="flex-1 h-12 rounded-xl border border-border text-[13px] font-black text-muted-foreground hover:bg-muted transition-all">Cancel</button>
                  <button onClick={handleAdjustStock} className="flex-1 h-12 rounded-xl bg-foreground text-background text-[13px] font-black shadow-lg hover:bg-foreground/90 transition-all">Update Stock Ledger</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {deleteFormTarget && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-sm rounded-[32px] shadow-2xl overflow-hidden p-8 text-center animate-in zoom-in-95 duration-200 border border-border">
              ...
              <h3 className="text-[20px] font-black text-foreground mb-2">Delete Asset?</h3>
              <p className="text-[13px] text-muted-foreground mb-8">Are you sure you want to remove <span className="font-black text-foreground">{deleteFormTarget.name}</span>? This action cannot be undone.</p>
              <div className="flex flex-col gap-3">
                <button onClick={() => handleDeleteItem(deleteFormTarget.id)} className="h-12 rounded-xl bg-red-500 text-white text-[14px] font-black hover:bg-red-600 transition-all shadow-lg shadow-red-200">Confirm Deletion</button>
                <button onClick={() => setDeleteFormTarget(null)} className="h-12 rounded-xl border border-border text-[14px] font-black text-muted-foreground hover:bg-muted transition-all">Cancel</button>
              </div>
            </div>
          </div>
        )}
        ...
        {viewPOTarget && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-card w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-primary">
              {/* Header */}
              <div className="bg-gradient-to-br from-primary to-indigo-600 p-8 flex justify-between items-start">
                <div>
                  <p className="text-[11px] text-primary/80 font-black uppercase tracking-widest mb-1">Purchase Order</p>
                  <h2 className="text-[28px] font-black text-white leading-tight tracking-tight">{viewPOTarget.orderNumber}</h2>
                  <p className="text-[12px] text-primary/80 mt-1">{new Date(viewPOTarget.createdAt).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`px-4 py-2 rounded-full text-[11px] font-black uppercase tracking-widest ${viewPOTarget.status === 'RECEIVED' ? 'bg-emerald-400/20 text-emerald-200 border border-emerald-400/30' :
                      viewPOTarget.status === 'SENT' ? 'bg-blue-400/20 text-blue-200 border border-blue-400/30' :
                        viewPOTarget.status === 'CANCELLED' ? 'bg-red-400/20 text-red-200 border border-red-400/30' :
                          'bg-foreground/10 text-white/80 border border-white/20'
                    }`}>{viewPOTarget.status}</span>
                  <button onClick={() => setViewPOTarget(null)} className="h-9 w-9 rounded-full bg-foreground/10 flex items-center justify-center text-white hover:bg-foreground/20 transition-all">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              </div>
              ...
              <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-muted/30 p-4 rounded-2xl border border-border/50">
                    <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mb-2">Supplier</p>
                    <p className="text-[15px] font-black text-foreground">{viewPOTarget.supplier?.name || '—'}</p>
                    {viewPOTarget.supplier?.email && <p className="text-[11px] text-muted-foreground mt-0.5">{viewPOTarget.supplier.email}</p>}
                  </div>
                  <div className="bg-muted/30 p-4 rounded-2xl border border-border/50">
                    <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mb-2">Order Total</p>
                    <p className="text-[22px] font-black text-primary">Rs. {Number(viewPOTarget.totalAmount).toLocaleString()}</p>
                  </div>
                </div>
                ...
                {viewPOTarget.notes && (
                  <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-2xl">
                    <p className="text-[10px] text-amber-500 font-black uppercase tracking-widest mb-1">Notes</p>
                    <p className="text-[13px] text-muted-foreground font-medium">{viewPOTarget.notes}</p>
                  </div>
                )}
                    ...
                    {['DRAFT', 'SENT', 'RECEIVED', 'CANCELLED'].map(s => (
                      <button
                        ...
                        onClick={() => updatePOStatus({ id: viewPOTarget.id, status: s }).unwrap()
                          ...
                          .catch(() => toast.error('Failed to update status'))}
                        className={`px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border ${viewPOTarget.status === s
              ...
              <div className="px-8 py-5 bg-muted/30 border-t border-border flex justify-between items-center">
                <p className="text-[11px] text-muted-foreground font-bold">ID: {viewPOTarget.id?.substring(0, 16)}...</p>
                <button onClick={() => setViewPOTarget(null)} className="h-10 px-6 rounded-xl border border-border text-[13px] font-black text-muted-foreground hover:bg-card transition-all">Close</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="fixed -left-[9999px] top-0 pointer-events-none opacity-0 select-none z-[-1]">
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-6 mb-12">
              <div className="bg-muted p-6 rounded-2xl border border-border shadow-sm">
                <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mb-1">Total SKUs</p>
                <p className="text-[28px] font-black text-foreground leading-none">{inventoryState.length}</p>
              </div>
              <div className="bg-emerald-500/10 p-6 rounded-2xl border border-emerald-500/20 shadow-sm">
                <p className="text-[10px] text-emerald-500/70 font-black uppercase tracking-widest mb-1">Healthy Stock</p>
                <p className="text-[28px] font-black text-emerald-500 leading-none">{inventoryState.filter(i => i.status === 'In Stock').length}</p>
              </div>
              <div className="bg-orange-500/10 p-6 rounded-2xl border border-orange-500/20 shadow-sm">
                <p className="text-[10px] text-orange-500/70 font-black uppercase tracking-widest mb-1">Critical Low</p>
                <p className="text-[28px] font-black text-orange-500 leading-none">{inventoryState.filter(i => ['Low Stock', 'Out of Stock'].includes(i.status)).length}</p>
              </div>
              <div className="bg-primary/10 p-6 rounded-2xl border border-primary/20 shadow-sm">
                <p className="text-[10px] text-primary/70 font-black uppercase tracking-widest mb-1">Stock Value</p>
                <p className="text-[28px] font-black text-primary leading-none">2.8M</p>
              </div>
            </div>
            ...
            {poPdfTarget && (
              <>
                ...
                <div style={{ marginTop: 'auto', paddingTop: 20, borderTop: '1px dashed #cbd5e1', display: 'flex', justifyContent: 'space-between' }}>
                  <p style={{ fontSize: 10, fontWeight: 900, color: '#0f172a' }}>
                    {user?.shopName || "All Fix Private Limited"} · {user?.shopAddress ? `${user.shopAddress}${user.shopCity ? `, ${user.shopCity}` : ''}` : "Colombo, Sri Lanka"}
                  </p>
