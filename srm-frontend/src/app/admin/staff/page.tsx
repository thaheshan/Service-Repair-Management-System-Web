...
import {
  ...
  Star, MoreVertical, ChevronLeft, ChevronRight, Plus
} from "lucide-react"
      ...
      <main className="flex-1 ml-[200px] flex flex-col">
        ...
        <div className="w-full max-w-[1280px] px-8 py-8 mx-auto flex flex-col min-h-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8 flex-1">
            {staffMock.map((staff) => (
              <div key={staff.id} className="bg-white rounded-[16px] border border-border p-6 flex flex-col items-center shadow-sm hover:border-[#4F46E5]/30 hover:shadow-md transition-all group">
                 {/* Avatar */}
                 <div className="relative mb-4">
                    <img 
                      src={`https://i.pravatar.cc/150?u=${staff.name.replace(/\s+/g, '')}`} 
                      alt={staff.name} 
                      className="h-[72px] w-[72px] rounded-full object-cover border-[3px] border-white shadow-md ring-1 ring-border group-hover:ring-[#4F46E5]/40 transition-all" 
                    />
                    <div className="absolute top-0 right-0 h-4 w-4 rounded-full bg-white flex items-center justify-center p-[2px]">
                       <div className={`h-full w-full rounded-full ${staff.status === 'Available' ? 'bg-green-500' : 'bg-red-500'}`} />
                    </div>
                 </div>

                 {/* Identity text */}
                 <h2 className="text-[16px] font-bold text-[#0F172A] tracking-tight">{staff.name}</h2>
                 <h3 className={`text-[12px] font-bold ${staff.titleColor} mt-1 mb-1.5 tracking-wide`}>{staff.title}</h3>
                 <p className="text-[12px] text-muted-foreground font-medium text-center">{staff.specialties}</p>

                 {/* Stats Row */}
                 <div className="flex items-center justify-between w-full mt-6 px-1">
                    <div className="flex flex-col items-center">
                       <span className="text-[20px] font-black leading-none text-[#0F172A] mb-1">{staff.active}</span>
                       <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Active</span>
                    </div>
                    <div className="h-8 w-px bg-border mx-2" />
                    <div className="flex flex-col items-center">
                       <span className="text-[20px] font-black leading-none text-[#0F172A] mb-1">{staff.week}</span>
                       <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">This Week</span>
                    </div>
                    <div className="h-8 w-px bg-border mx-2" />
                    <div className="flex flex-col items-center">
                       <span className="text-[20px] font-black leading-none text-[#0F172A] mb-1 flex items-center gap-1"><Star className="h-4 w-4 text-[#F59E0B] fill-[#F59E0B]" /> {staff.rating.toFixed(1)}</span>
                       <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Rating</span>
                    </div>
                 </div>

                 <div className={`text-[11px] font-bold tracking-widest uppercase mt-6 mb-6 ${staff.statusColor}`}>
                   {staff.status}
                 </div>
                 ...
                 <div className="flex w-full gap-2 mt-auto">
                    ...
                    <button onClick={() => alert("Open Assignment workflow for: " + staff.name)} className="flex-[1.5] h-9 rounded-lg bg-[#4F46E5] text-[13px] font-bold text-white shadow-sm hover:bg-[#4338CA] transition-colors focus:outline-none">
                      Assign Repair
                    </button>
                    <button onClick={() => alert("More options...")} className="flex shrink-0 items-center justify-center w-9 h-9 rounded-lg border border-border bg-white text-muted-foreground hover:bg-muted hover:text-[#0F172A] transition-colors focus:outline-none shadow-sm">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 py-4 border-t border-border mt-auto w-full">
            <span className="text-[13px] text-muted-foreground font-medium">
              Showing <span className="font-bold text-[#0F172A]">1-8</span> of <span className="font-bold text-[#0F172A]">12</span> team members
            </span>
