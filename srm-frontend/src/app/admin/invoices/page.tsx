            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto relative">
                <button 
                  ...
                  className={`flex items-center gap-2 h-10 px-4 rounded-lg border text-[13px] font-bold transition-all focus:outline-none whitespace-nowrap ${isFiltersOpen ? 'bg-[#4F46E5] border-[#4F46E5] text-white shadow-lg' : 'bg-white border-border text-[#0F172A] hover:bg-muted shadow-sm hover:shadow'}`}
                >
                ...
                {isFiltersOpen && (
                   <div className="absolute top-12 right-0 w-[320px] bg-white rounded-2xl shadow-2xl border border-border/80 z-[60] animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden">
                      <div className="p-5 border-b border-border bg-[#F8FAFC] flex items-center justify-between">
                        <h3 className="text-[15px] font-black text-[#0F172A]">Advanced Filters</h3>
                        <button onClick={clearFilters} className="text-[11px] font-bold text-[#4F46E5] hover:underline">Clear All</button>
                      </div>
                      <div className="p-5 space-y-6 max-h-[450px] overflow-y-auto">
                        <section>
                          <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-3">Invoice Type</label>
                          <div className="flex flex-wrap gap-2">
                            {INVOICE_TYPES.map(t => (
                              <button key={t} onClick={() => toggleFilter(t, filterTypes, setFilterTypes)} className={`px-3 py-1.5 rounded-lg text-[11px] font-bold border transition-colors ${filterTypes.includes(t) ? 'bg-[#4F46E5] border-[#4F46E5] text-white' : 'border-border text-muted-foreground hover:bg-muted'}`}>
                                {t === 'client_repair' ? 'Repair' : 'Inventory/Sale'}
                              </button>
                            ))}
                          </div>
                        </section>
                        ...
                        <section>
                          <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-3">Status</label>
                          <div className="flex flex-wrap gap-2">
                            {INVOICE_STATUSES.map(s => (
                              <button key={s} onClick={() => toggleFilter(s, filterStatuses, setFilterStatuses)} className={`px-3 py-1.5 rounded-lg text-[11px] font-bold border transition-colors ${filterStatuses.includes(s) ? 'bg-[#4F46E5] border-[#4F46E5] text-white' : 'border-border text-muted-foreground hover:bg-muted'}`}>
                                {s}
                              </button>
                            ))}
                          </div>
                        ...
                        <section>
                          <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-3">Staff / Technician</label>
                          <div className="flex flex-wrap gap-2">
                            {STAFF_LIST.map(st => (
                              <button key={st} onClick={() => toggleFilter(st, filterStaff, setFilterStaff)} className={`px-3 py-1.5 rounded-lg text-[11px] font-bold border transition-colors ${filterStaff.includes(st) ? 'bg-[#4F46E5] border-[#4F46E5] text-white' : 'border-border text-muted-foreground hover:bg-muted'}`}>
                                {st}
                              </button>
                            ))}
                          </div>
                        ...
                        <section>
                          <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-3">Device Category</label>
                          <div className="flex flex-wrap gap-2">
                            {DEVICE_TYPES.map(d => (
                              <button key={d} onClick={() => toggleFilter(d, filterDevices, setFilterDevices)} className={`px-3 py-1.5 rounded-lg text-[11px] font-bold border transition-colors ${filterDevices.includes(d) ? 'bg-[#4F46E5] border-[#4F46E5] text-white' : 'border-border text-muted-foreground hover:bg-muted'}`}>
                                {d}
                              </button>
                            ))}
                          </div>
                        ...
                        <section>
                          <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-3">Amount Range (LKR)</label>
                          <div className="space-y-4">
                            <div className="flex items-center gap-3">
                              <input type="number" value={filterAmountRange.min} onChange={e => setFilterAmountRange(p => ({...p, min: +e.target.value}))} className="w-full h-9 rounded-lg border border-border px-2 text-[12px] font-bold outline-none focus:border-[#4F46E5]" />
                              <span className="text-muted-foreground text-[12px]">to</span>
                              <input type="number" value={filterAmountRange.max} onChange={e => setFilterAmountRange(p => ({...p, max: +e.target.value}))} className="w-full h-9 rounded-lg border border-border px-2 text-[12px] font-bold outline-none focus:border-[#4F46E5]" />
                            </div>
                          </div>
                        </section>

                        <section>
                          <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-3">Date Range</label>
                          <div className="flex flex-col gap-2">
                            <input type="date" value={filterDateFrom} onChange={e => setFilterDateFrom(e.target.value)} className="w-full h-9 rounded-lg border border-border px-3 text-[12px] font-bold outline-none focus:border-[#4F46E5]" />
                            <input type="date" value={filterDateTo} onChange={e => setFilterDateTo(e.target.value)} className="w-full h-9 rounded-lg border border-border px-3 text-[12px] font-bold outline-none focus:border-[#4F46E5]" />
                          </div>
                        </section>
                      </div>
                      <div className="p-4 bg-[#F8FAFC] border-t border-border">
                        <button onClick={() => setIsFiltersOpen(false)} className="w-full h-10 rounded-xl bg-[#0F172A] text-white text-[12px] font-bold hover:bg-black transition-colors shadow-lg">Apply Filters</button>
                      </div>
                   </div>
                )}
                 ...
                 <div className="relative">
                   <button 
                     ...
                     className="flex items-center justify-between gap-2 w-48 h-10 px-4 rounded-lg border border-border bg-white text-[13px] font-bold text-[#0F172A] hover:bg-muted shadow-sm transition-all focus:outline-none"
                   >
                     <span className="text-muted-foreground font-medium text-[12px] truncate">Sort by: {SORT_OPTIONS.find(o => o.value === sortKey)?.label.split(' (')[0]}</span> <ChevronDown className="h-4 w-4 text-muted-foreground shrink-0" />
                   </button>
                   {isSortOpen && (
