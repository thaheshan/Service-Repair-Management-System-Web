                  ...
                  <div className="flex items-center gap-2 mb-6 w-full overflow-x-auto pb-2 md:pb-0">
                     {[
                        ...
                        <button
                           ...
                           className={`h-9 px-6 rounded-full text-[13px] font-bold transition-all focus:outline-none whitespace-nowrap flex items-center gap-2 ${activeTab === tab.id ? 'bg-[#4F46E5] text-white shadow-md' : 'text-muted-foreground hover:text-[#0F172A] hover:bg-black/5'
                              }`}
                        >
                           {tab.label}
                           <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-black ${activeTab === tab.id ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'}`}>
                              {tab.count}
                           </span>
                        </button>
                     ))}
                           ...
                           onClick={async () => {
                              ...
                              try {
                                 await createInvoiceMutation({
                                    name, // Backend can use this for walk-ins
                                    phone,
                                    amount: finalAmount,
                                    shopId: user?.shopId,
