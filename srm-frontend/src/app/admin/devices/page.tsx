...
export default function DevicesPage() {
  ...
  const allCustomers = useMemo(() => {
    const raw = (customersResponse as any)?.customers || (customersResponse as any)?.data || [];
    return raw;
  }, [customersResponse])
  ...
  const handleDownloadDevicePdf = async () => {
    ...
    // Small delay for DOM sync
    await new Promise(r => setTimeout(r, 300));

    try {
      // 1. Capture computed styles of all elements to "bake" them in
      const elements = Array.from(deviceDetailRef.current.getElementsByTagName("*"));
      const computedStyles = elements.map(el => {
        const style = window.getComputedStyle(el);
        // We only need a few critical properties to avoid the lab parser error while keeping design
        return {
      ...
      const canvas = await html2canvas(deviceDetailRef.current, { 
        ...
        onclone: (clonedDoc) => {
          ...
          clonedElements.forEach((el, i) => {
             ...
             // Manually apply styles to bypass the CSS parser for the whole document
             ...
             cel.style.fontFamily = "Arial, sans-serif"; // Force safe font
          ...
          // Remove all style/link tags so the parser doesn't even run
  ...
  const handleDownloadInventoryPdf = async () => {
    ...
    try {
      ...
      // 1. Capture computed styles of all elements
            ...
            <div className="p-6 space-y-4 max-h-[78vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.model} onChange={e => setForm(p => ({ ...p, model: e.target.value }))} placeholder="e.g. iPhone 14 Pro" className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-bold focus:outline-none focus:border-[#4F46E5] focus:ring-4 focus:ring-[#4F46E5]/5 transition-all" />
                </div>
                <div>
                  <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Brand *</label>
                  <select value={form.brand} onChange={e => setForm(p => ({ ...p, brand: e.target.value }))} className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-bold focus:outline-none focus:border-[#4F46E5] bg-white appearance-none outline-none">
              ...
              {form.brand === "Other" && (
                <div className="animate-in slide-in-from-top-2 duration-200">
                  ...
                  <input value={customBrand} onChange={e => setCustomBrand(e.target.value)} placeholder="Type brand name..." className="w-full h-11 rounded-xl border border-primary bg-indigo-50/20 px-4 text-[13px] font-bold focus:outline-none focus:border-[#4F46E5]" />
                </div>
              )}
              {/* Row 2: Type */}
              <div>
                <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Device Type</label>
                <select value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value as DeviceType }))} className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-bold focus:outline-none focus:border-[#4F46E5] bg-white appearance-none outline-none">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.imei} onChange={e => setForm(p => ({ ...p, imei: e.target.value }))} placeholder="15-digit IMEI (Optional)" className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-mono focus:outline-none focus:border-[#4F46E5]" />
                </div>
                <div>
                  <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Serial Number</label>
                  <input value={form.serialNo} onChange={e => setForm(p => ({ ...p, serialNo: e.target.value }))} placeholder="Manufacturer serial (Optional)" className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-mono focus:outline-none focus:border-[#4F46E5]" />
                </div>
              </div>
              {/* Price Field */}
              {user?.role !== 'TECHNICIAN' && (
              ...
              <div>
                ...
                {addSelectedCustomer ? (
                  <div className="flex items-center gap-3 p-3 rounded-xl border-2 border-emerald-400 bg-emerald-50">
                    <div className="h-9 w-9 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 text-[13px] font-black">{addSelectedCustomer.name.charAt(0)}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-black text-[#0F172A] truncate">{addSelectedCustomer.name}</p>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full h-11 pl-10 pr-4 rounded-xl border border-border text-[13px] font-bold focus:outline-none focus:border-[#4F46E5] transition-all"
                    />
                    {showCustomerDropdown && filteredCustomers.length > 0 && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-border rounded-xl shadow-xl z-[110] max-h-[200px] overflow-y-auto">
                        {filteredCustomers.map((c: any) => (
                          <button key={c.id} onClick={() => { setAddSelectedCustomer({ id: c.id, name: c.name, phone: c.phone || '—' }); setShowCustomerDropdown(false); setAddCustomerSearch('') }} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#F8FAFC] transition-colors text-left border-b border-border/40 last:border-0">
                            <div className="h-8 w-8 rounded-full bg-[#EEF2FF] flex items-center justify-center text-[#4F46E5] text-[11px] font-black shrink-0">{c.name?.charAt(0) || '?'}</div>
                            <div><p className="text-[13px] font-bold text-[#0F172A]">{c.name}</p><p className="text-[11px] text-muted-foreground">{c.phone || 'No phone'}</p></div>
                          </button>
                        ))}
                      </div>
                    )}
          ...
          <div className="bg-white w-full max-w-[560px] rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-6 border-b border-border bg-[#F8FAFC]">
              <div>
                ...
                <p className="text-[12px] text-muted-foreground font-medium mt-0.5">Updating: <span className="font-bold text-[#0F172A]">{editDevice.name}</span></p>
              </div>
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Device Model / Name</label>
                  <input value={editForm.model} onChange={e => setEditForm(p => ({ ...p, model: e.target.value }))} className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-bold focus:outline-none focus:border-[#4F46E5]" />
                </div>
                <div>
                  <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Brand</label>
                  <select value={editForm.brand} onChange={e => setEditForm(p => ({ ...p, brand: e.target.value }))} className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-bold focus:outline-none focus:border-[#4F46E5] bg-white appearance-none">
                    <option value="Apple">Apple</option><option value="Samsung">Samsung</option><option value="Huawei">Huawei</option>
                    <option value="Xiaomi">Xiaomi</option><option value="Oppo">Oppo</option><option value="Vivo">Vivo</option>
                    <option value="Sony">Sony</option><option value="Nokia">Nokia</option><option value="Google">Google</option><option value="Other">Other</option>
                  </select>
              ...
              <div>
                <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Device Type</label>
                <select value={editForm.type} onChange={e => setEditForm(p => ({ ...p, type: e.target.value as DeviceType }))} className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-bold focus:outline-none focus:border-[#4F46E5] bg-white appearance-none">
                  {TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                </select>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">IMEI Number</label>
                  <input value={editForm.imei} onChange={e => setEditForm(p => ({ ...p, imei: e.target.value }))} placeholder="15-digit IMEI" className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-mono focus:outline-none focus:border-[#4F46E5]" />
                </div>
                <div>
                  <label className="block text-[11px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Serial Number</label>
                  <input value={editForm.serialNo} onChange={e => setEditForm(p => ({ ...p, serialNo: e.target.value }))} placeholder="Manufacturer serial" className="w-full h-11 rounded-xl border border-border px-4 text-[13px] font-mono focus:outline-none focus:border-[#4F46E5]" />
                </div>
              </div>
              {/* Price Field */}
              <div>
