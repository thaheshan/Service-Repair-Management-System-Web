    ...
    return (
      <div className="flex bg-background h-screen overflow-hidden">
        ...
        <div className="flex-1 lg:ml-[200px] ml-0 flex flex-col items-center justify-center p-8">
          ...
          <button onClick={() => router.push("/admin/repairs")} className="h-10 px-6 rounded-xl bg-primary text-primary-foreground font-bold hover:bg-primary/90 transition-colors">Go back to Repairs</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex bg-background h-screen overflow-hidden">
      ...
      <div className="flex-1 lg:ml-[200px] ml-0 flex flex-col min-w-0">
        ...
        <main className="flex-1 flex flex-col pt-0 overflow-y-auto bg-muted/20">
          <div className="w-full max-w-[1280px] px-8 py-8 mx-auto flex flex-col">
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
              <div>
                <div className="flex items-center gap-3 mb-3">
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-md bg-primary/10 text-primary text-[11px] font-bold tracking-wide">
              ...
              <div className="flex items-center gap-3 shrink-0 relative">
                <button onClick={() => router.push("/admin/schedule")} className="flex items-center gap-1.5 h-9 px-4 rounded-lg border border-primary text-primary bg-card text-[13px] font-bold hover:bg-primary/10 transition-colors focus:outline-none shadow-sm">
                  <Calendar className="h-4 w-4" /> Reschedule
                </button>
                <div className="relative">
                  <button 
                    onClick={() => setIsReassignDropdownOpen(!isReassignDropdownOpen)}
                   className="flex items-center gap-1.5 h-9 px-4 rounded-lg border border-emerald-500 text-emerald-600 dark:text-emerald-400 bg-card text-[13px] font-bold hover:bg-emerald-500/10 transition-colors focus:outline-none shadow-sm"
                  >
                    <User className="h-4 w-4" /> Reassign
                  </button>
                  {isReassignDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-56 rounded-xl bg-popover shadow-xl border border-border z-50 p-2 animate-in fade-in zoom-in-95 duration-200">
                      ...
                      <div className="max-h-60 overflow-y-auto custom-scrollbar">
                        ...
                        {technicians.map((tech: any) => (
                          ...
                          >
                            <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary text-[11px] font-black shrink-0 border border-primary/10">
                ...
                <button onClick={() => setIsCancelModalOpen(true)} className="flex items-center gap-1.5 h-9 px-4 rounded-lg border border-destructive text-destructive bg-card text-[13px] font-bold hover:bg-destructive/10 transition-colors focus:outline-none shadow-sm">
            ...
            {!isCompleted ? (
              <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/50 rounded-xl p-4 flex items-center justify-between mb-8 shadow-sm">
                <div className="flex items-center gap-4">
                  <div className="h-10 w-10 rounded-full bg-amber-100 dark:bg-amber-900/50 flex items-center justify-center">
                    <Wrench className="h-5 w-5 text-amber-600 dark:text-amber-500" />
                  </div>
                  <div>
                    <h3 className="text-[14px] font-bold text-amber-800 dark:text-amber-400">Repair is currently {repair.status.replace(/_/g, ' ').toLowerCase()}</h3>
                    <p className="text-[13px] text-amber-700 dark:text-amber-400 font-medium">Estimated cost: <span className="font-bold">Rs. {repair.estimatedCost?.toLocaleString() || '0'}</span></p>
                  </div>
                </div>
                <button 
                  disabled={isUpdating}
                  onClick={handleToggleComplete} 
                  className={`h-9 px-5 rounded-lg bg-primary text-primary-foreground text-[13px] font-bold hover:bg-primary/90 shadow-md transition-colors focus:outline-none ${isUpdating ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {isUpdating ? "Updating..." : "Mark Complete"}
                </button>
              </div>
            ) : (
              <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-900/50 rounded-xl p-4 flex items-center justify-between mb-8 shadow-sm">
                <div className="flex items-center gap-4">
                  <div className="h-10 w-10 rounded-full bg-emerald-100 dark:bg-emerald-900/50 flex items-center justify-center">
                    <CheckCircle className="h-5 w-5 text-emerald-600 dark:text-emerald-500" />
                  </div>
                  <div>
                    <h3 className="text-[14px] font-bold text-emerald-800 dark:text-emerald-400">Repair {repair.status.replace(/_/g, ' ').toLowerCase()}</h3>
                    <p className="text-[13px] text-emerald-800 dark:text-emerald-400 font-medium">Final cost: <span className="font-bold">Rs. {repair.finalCost?.toLocaleString() || repair.estimatedCost?.toLocaleString() || '0'}</span></p>
                  </div>
                </div>
                <button 
                  disabled={isUpdating}
                  onClick={handleToggleComplete} 
                  className={`h-9 px-5 rounded-lg border border-emerald-500 text-emerald-600 dark:text-emerald-400 bg-card text-[13px] font-bold hover:bg-emerald-500/10 shadow-sm transition-colors focus:outline-none ${isUpdating ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-8">
              ...
              <div className="flex flex-col gap-6">
                ...
                {/* Timeline */}
                <section className="bg-card rounded-xl shadow-sm border border-border p-7">
                  <h2 className="text-[15px] font-bold text-foreground mb-8 flex items-center gap-2">
                    <Clock className="h-[18px] w-[18px] text-primary" /> Task Timeline
                  </h2>
                    ...
                    {repair.timeline?.length > 0 ? (
                      repair.timeline.map((event: any, idx: number) => (
                        <div key={event.id} className="relative flex items-start gap-5">
                          ...
                          <div className={`relative z-10 flex h-7 w-7 items-center justify-center rounded-full border-2 ${event.type === 'CREATED' ? 'border-emerald-500 bg-emerald-100 dark:bg-emerald-900/50' : 'border-primary bg-primary/10'}`}>
                            {event.type === 'CREATED' ? (
                              <Plus className="h-4 w-4 text-emerald-600 dark:text-emerald-400" strokeWidth={3} />
                            ) : (
                ...
                <section className="bg-card rounded-xl shadow-sm border border-border p-7">
                  <h2 className="text-[15px] font-bold text-foreground mb-6 flex items-center gap-2">
                    <PenTool className="h-[18px] w-[18px] text-primary" /> Technician Notes
                  </h2>
                  ...
                  <div className="relative">
                    <textarea 
                      ...
                      className="w-full p-4 text-[13px] border border-border rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none bg-transparent"
                    />
                    
                    {showMentionDropdown && (
                      <div className="absolute left-0 bottom-full mb-2 w-64 rounded-xl bg-popover border border-border shadow-xl z-50 p-2 max-h-48 overflow-y-auto custom-scrollbar">
                  ...
                  <div className="flex justify-end mt-3">
                    <button 
                      ...
                      className={`h-8 px-5 rounded-lg bg-primary text-[12px] font-bold text-primary-foreground hover:bg-primary/90 transition-all ${isAddingNote || !taskNote.trim() ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                ...
                <section className="bg-card rounded-xl shadow-sm border border-border p-7">
                  <div className="flex items-center justify-between mb-5">
                    <h2 className="text-[15px] font-bold text-foreground flex items-center gap-2">
                      <ImageIcon className="h-[18px] w-[18px] text-primary" /> Device Photos
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 h-8 px-3 rounded-md bg-primary/10 text-primary text-[12px] font-bold hover:bg-primary/20 transition-colors"
                    >
                  ...
                  {repair.photos?.length > 0 ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {repair.photos.map((photo: any, i: number) => (
                        <div
                          key={photo.id || i}
                          className="relative aspect-square rounded-xl overflow-hidden border border-border bg-muted group hover:ring-2 hover:ring-primary transition-all cursor-pointer"
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="bg-card rounded-xl shadow-sm border border-border p-6">
                  ...
                  <p className="font-bold text-primary">{repair.customer?.name}</p>
      ...
      {isCancelModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="relative w-full max-w-md rounded-2xl bg-card p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            <h2 className="text-xl font-bold mb-2 text-foreground">Cancel Task?</h2>
            ...
            <div className="flex gap-4">
              ...
              <button 
                ...
                className="flex-1 h-11 rounded-xl bg-destructive text-destructive-foreground font-bold hover:bg-destructive/90 transition-colors shadow-lg dark:shadow-none"
              >
                Confirm Cancel
              </button>
            </div>
      ...
      {photoToDelete && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-card w-full max-w-md rounded-2xl p-6 shadow-xl animate-in zoom-in-95 duration-200">
